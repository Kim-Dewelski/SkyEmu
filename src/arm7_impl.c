...
// JIT implementation
static void arm7_jit_data_processing(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_multiply(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_multiply_long(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_single_data_swap(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_branch_exchange(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_branch_link_exchange(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_half_word_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_single_word_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_undefined(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_block_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_branch(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_branch(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_coproc_data_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_coproc_data_op(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_coproc_reg_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_software_interrupt(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_mrs(arm7_t* cpu, uint32_t opcode);
static void arm7_jit_msr(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_clz(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_double_word_transfer(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_mov_shift_reg(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_add_sub(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_mov_cmp_add_sub_imm(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_alu_op(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_hi_reg_op(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_hi_reg_op(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_pc_rel_ldst(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_reg_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_ldst_bh(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_imm_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_imm_off_ldst_bh(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_stack_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_load_addr(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_add_off_sp(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_push_pop_reg(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_mult_ldst(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_cond_branch(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_soft_interrupt(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_branch(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_long_branch_link(arm7_t* cpu, uint32_t opcode);
static void arm7t_jit_unknown(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_mult_ldst(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_push_pop_reg(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_stack_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_imm_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_reg_off_ldst(arm7_t* cpu, uint32_t opcode);
static void arm9t_jit_pc_rel_ldst(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_qadd_qsub(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_signed_halfword_multiply(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_single_word_transfer(arm7_t* cpu, uint32_t opcode);
static void arm9_jit_block_transfer(arm7_t* cpu, uint32_t opcode);
...
const static arm7_instruction_t arm7_instruction_classes[]={
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc0010oooSnnnnddddrrrrOOOOOOOO"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00111ooSnnnnddddrrrrOOOOOOOO"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00110oo1nnnnddddrrrrOOOOOOOO"},
   //These duplications are to handle disambiguating bit 5 and 7 set to ones for DP 
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc0000oooSnnnnddddsssssss0mmmm"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc0000oooSnnnnddddssss0tt1mmmm"},
   //Handle TST, TEQ, CMP, CMN must set S case
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00011ooSnnnnddddsssssss0mmmm"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00011ooSnnnnddddssss0tt1mmmm"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00010oo1nnnnddddssss0tt1mmmm"},
   {arm7_data_processing,      arm7_jit_data_processing,      "DP",        "cccc00010oo1nnnnddddsssssss0mmmm"},

   {arm7_multiply,             arm7_jit_multiply,             "MUL",       "cccc000000ASddddnnnnssss1001mmmm"},
   {arm7_multiply_long,        arm7_jit_multiply_long,        "MLONG",     "cccc00001UASddddnnnnssss1001mmmm"},
   {arm7_single_data_swap,     arm7_jit_single_data_swap,     "SDS",       "cccc00010B00nnnndddd00001001mmmm"},
   {arm7_branch_exchange,      arm7_jit_branch_exchange,      "BX",        "cccc000100101111111111110001nnnn"},

   {arm7_undefined,            arm7_jit_undefined,            "LDRD/STRD", "cccc000PUIW0nnnnddddoooo11S1oooo"},
   {arm7_half_word_transfer,   arm7_jit_half_word_transfer,   "HDT(h)",    "cccc000PUIWLnnnndddd00001011mmmm"},
   {arm7_half_word_transfer,   arm7_jit_half_word_transfer,   "HDT(sb)",   "cccc000PUIW1nnnnddddOOOO1101OOOO"},
   {arm7_half_word_transfer,   arm7_jit_half_word_transfer,   "HDT(sh)",   "cccc000PUIW1nnnnddddOOOO1111OOOO"},
   {arm7_single_word_transfer, arm7_jit_single_word_transfer, "SDT",       "cccc010PUBWLnnnnddddOOOOOOOOOOOO"},
   {arm7_single_word_transfer, arm7_jit_single_word_transfer, "SDT",       "cccc011PUBWLnnnnddddOOOOOOO0mmmm"},

   {arm7_undefined,            arm7_jit_undefined,            "UDEF",      "cccc011--------------------1----"},
   {arm7_block_transfer,       arm7_jit_block_transfer,       "BDT",       "cccc100PUSWLnnnnllllllllllllllll"},
   {arm7_branch,               arm7_jit_branch,               "B",         "cccc1010OOOOOOOOOOOOOOOOOOOOOOOO"},
   {arm7_branch,               arm7_jit_branch,               "BL",        "cccc1011OOOOOOOOOOOOOOOOOOOOOOOO"},
   {arm7_coproc_data_transfer, arm7_jit_coproc_data_transfer, "CDT",       "cccc110PUNWLnnnndddd####OOOOOOOO"},
   {arm7_coproc_data_op,       arm7_jit_coproc_data_op,       "CDO",       "cccc1110oooonnnndddd####ppp0mmmm"},
   {arm7_coproc_reg_transfer,  arm7_jit_coproc_reg_transfer,  "CRT",       "cccc1110oooLnnnndddd####ppp1mmmm"},
   {arm7_software_interrupt,   arm7_jit_software_interrupt,   "SWI",       "cccc1111------------------------"},
   {arm7_mrs,                  arm7_jit_mrs,                  "MRS",       "cccc00010P001111dddd000000000000"},
   {arm7_msr,                  arm7_jit_msr,                  "MSR",       "cccc00010P10100F111100000000mmmm"},
   {arm7_msr,                  arm7_jit_msr,                  "MSR",       "cccc00110P10100F1111oooooooooooo"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN1",  "cccc000001--------------1001----"}, 
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN2",  "cccc00011---------------1001----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN3",  "cccc00010-1-------------1001----"}, 
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN4",  "cccc00010-01------------1001----"}, 
   // Handle invalid opcode space in DP
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN6",  "cccc00010-00------------01-0----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN7",  "cccc00010-00------------0010----"},
   {arm7_undefined,            arm7_jit_undefined,            "QADD/QSUB", "cccc00010oo0nnnndddd00000101mmmm"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWN8",  "cccc00010-00------------00-1----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNN",  "cccc00010-00------------0111----"},
   {arm7_undefined,            arm7_jit_undefined,            "SMLA" ,     "cccc00010oo0ddddnnnnssss1yx0mmmm"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNA",  "cccc00010110------------01-0----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNB",  "cccc00010110------------0010----"},
   {arm7_undefined,            arm7_jit_undefined,            "CLZ",       "cccc000101101111DDDD11110001MMMM"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNC",  "cccc00010110------------0111----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWND",  "cccc00010110------------0011----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNE",  "cccc00010010------------0111----"},
   {arm7_undefined,            arm7_jit_undefined,            "BLX",       "cccc000100101111111111110011nnnn"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNG",  "cccc00010010------------01-0----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNH",  "cccc00010010------------0010----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNI",  "cccc00110-000000000000001-------"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNJ",  "cccc00110-0000000000000001------"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNK",  "cccc00110-00000000000000001-----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNL",  "cccc00110-000000000000000001----"},
   {arm7_undefined,            arm7_jit_undefined,            "UNKNOWNM",  "----00110-00------------0000----"},
   {NULL},
...
const static arm7_instruction_t arm9_instruction_classes[]={
   ...
   {arm9_double_word_transfer,     arm9_jit_double_word_transfer,     "LDRD/STRD", "cccc000PUIW0nnnnddddoooo11S1oooo"},
   {arm7_half_word_transfer,       arm7_jit_half_word_transfer,       "HDT(h)",    "cccc000PUIWLnnnndddd00001011mmmm"},
   {arm7_half_word_transfer,       arm7_jit_half_word_transfer,       "HDT(sb)",   "cccc000PUIW1nnnnddddOOOO1101OOOO"},
   {arm7_half_word_transfer,       arm7_jit_half_word_transfer,       "HDT(sh)",   "cccc000PUIW1nnnnddddOOOO1111OOOO"},
   {arm9_single_word_transfer,     arm9_jit_single_word_transfer,     "SDT",       "cccc010PUBWLnnnnddddOOOOOOOOOOOO"},
   {arm9_single_word_transfer,     arm9_jit_single_word_transfer,     "SDT",       "cccc011PUBWLnnnnddddOOOOOOO0mmmm"},

   {arm7_undefined,                arm7_jit_undefined,                "UDEF",      "cccc011--------------------1----"},
   {arm9_block_transfer,           arm9_jit_block_transfer,           "BDT",       "cccc100PUSWLnnnnllllllllllllllll"},
   {arm9_branch,                   arm9_jit_branch,                   "B",         "cccc1010OOOOOOOOOOOOOOOOOOOOOOOO"},
   {arm9_branch,                   arm9_jit_branch,                   "BL",        "cccc1011OOOOOOOOOOOOOOOOOOOOOOOO"},
   {arm7_coproc_data_transfer,     arm7_jit_coproc_data_transfer,     "CDT",       "cccc110PUNWLnnnndddd####OOOOOOOO"},
   {arm7_coproc_data_op,           arm7_jit_coproc_data_op,           "CDO",       "cccc1110oooonnnndddd####ppp0mmmm"},
   {arm7_coproc_reg_transfer,      arm7_jit_coproc_reg_transfer,      "CRT",       "cccc1110oooLnnnndddd####ppp1mmmm"},
   {arm7_software_interrupt,       arm7_jit_software_interrupt,       "SWI",       "cccc1111------------------------"},
   ...
   {arm9_qadd_qsub,                arm9_jit_qadd_qsub,                "QADD/QSUB", "cccc00010oo0nnnndddd00000101mmmm"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWN8",  "cccc00010-00------------00-1----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNN",  "cccc00010-00------------0111----"},
   {arm9_signed_halfword_multiply, arm9_jit_signed_halfword_multiply, "SMLA" ,     "cccc00010oo0ddddnnnnssss1yx0mmmm"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNA",  "cccc00010110------------01-0----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNB",  "cccc00010110------------0010----"},
   {arm9_clz,                      arm9_jit_clz,                      "CLZ",       "cccc000101101111DDDD11110001MMMM"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNC",  "cccc00010110------------0111----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWND",  "cccc00010110------------0011----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNE",  "cccc00010010------------0111----"},
   {arm9_branch_link_exchange,     arm9_jit_branch_link_exchange,     "BLX",       "cccc000100101111111111110011nnnn"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNG",  "cccc00010010------------01-0----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNH",  "cccc00010010------------0010----"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNI",  "cccc00110-000000000000001-------"},
   {arm7_undefined,                arm7_jit_undefined,                "UNKNOWNJ",  "cccc00110-0000000000000001------"},
...
const static arm7_instruction_t arm7t_instruction_classes[]={
   {arm7t_mov_shift_reg,       arm7t_jit_mov_shift_reg,       "LSL",         "00000OOOOOsssddd"},
   {arm7t_mov_shift_reg,       arm7t_jit_mov_shift_reg,       "LSR",         "00001OOOOOsssddd"},
   {arm7t_mov_shift_reg,       arm7t_jit_mov_shift_reg,       "ASR",         "00010OOOOOsssddd"},
   {arm7t_add_sub,             arm7t_jit_add_sub,             "ADD",         "00011I0nnnsssddd"},
   {arm7t_add_sub,             arm7t_jit_add_sub,             "SUB",         "00011I1nnnsssddd"},
   {arm7t_mov_cmp_add_sub_imm, arm7t_jit_mov_cmp_add_sub_imm, "MCASIMM",     "001oodddOOOOOOOO"},
   {arm7t_alu_op,              arm7t_jit_alu_op,              "ALU",         "010000oooosssddd"},
   {arm7t_hi_reg_op,           arm7t_jit_hi_reg_op,           "HROP",        "010001oohHsssddd"},
   {arm7t_pc_rel_ldst,         arm7t_jit_pc_rel_ldst,         "PCRLD",       "01001dddOOOOOOOO"},
   {arm7t_reg_off_ldst,        arm7t_jit_reg_off_ldst,        "LDST[RD]",    "0101LB0ooobbbddd"},
   {arm7t_ldst_bh,             arm7t_jit_ldst_bh,             "SLDST[RD]",   "0101HS1ooobbbddd"},
   {arm7t_imm_off_ldst,        arm7t_jit_imm_off_ldst,        "LDST[IMM]",   "011BLOOOOObbbddd"},
   {arm7t_imm_off_ldst_bh,     arm7t_jit_imm_off_ldst_bh,     "SLDSTH[IMM]", "1000LOOOOObbbddd"},
   {arm7t_stack_off_ldst,      arm7t_jit_stack_off_ldst,      "LDST[SP]",    "1001LdddOOOOOOOO"},
   {arm7t_load_addr,           arm7t_jit_load_addr,           "LDADDR",      "1010SdddOOOOOOOO"},
   {arm7t_add_off_sp,          arm7t_jit_add_off_sp,          "SP+=OFF",     "10110000SOOOOOOO"},
   {arm7t_push_pop_reg,        arm7t_jit_push_pop_reg,        "PUSHPOPREG",  "1011L10Rllllllll"},
   {arm7t_mult_ldst,           arm7t_jit_mult_ldst,           "MLDST",       "1100Lbbbllllllll"},
   // Conditional branches cant branch on condition 1111
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "11010cccOOOOOOOO"},
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "110110ccOOOOOOOO"},
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "1101110cOOOOOOOO"},
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "11011110OOOOOOOO"},
   {arm7t_soft_interrupt,      arm7t_jit_soft_interrupt,      "SWI",         "11011111OOOOOOOO"},
   {arm7t_branch,              arm7t_jit_branch,              "B",           "11100OOOOOOOOOOO"},
   {arm7t_long_branch_link,    arm7t_jit_long_branch_link,    "BL",          "1111HOOOOOOOOOOO"},
   {arm7t_long_branch_link,    arm7t_jit_long_branch_link,    "BLX",         "11101OOOOOOOOOOO"},
   //Empty Opcode Space
   {arm7t_unknown,             arm7t_jit_unknown,             "UNKNOWN1",    "1011--1---------"},
   {arm7t_unknown,             arm7t_jit_unknown,             "UNKNOWN2",    "10110001--------"},
   {arm7t_unknown,             arm7t_jit_unknown,             "UNKNOWN3",    "1011100---------"},
   {NULL},
};  
...
const static arm7_instruction_t arm9t_instruction_classes[]={
   ...
   {arm7t_mov_cmp_add_sub_imm, arm7t_jit_mov_cmp_add_sub_imm, "MCASIMM",     "001oodddOOOOOOOO"},
   {arm7t_alu_op,              arm7t_jit_alu_op,              "ALU",         "010000oooosssddd"},
   {arm9t_hi_reg_op,           arm9t_jit_hi_reg_op,           "HROP",        "010001oohHsssddd"},
   {arm9t_pc_rel_ldst,         arm9t_jit_pc_rel_ldst,         "PCRLD",       "01001dddOOOOOOOO"},
   {arm9t_reg_off_ldst,        arm9t_jit_reg_off_ldst,        "LDST[RD]",    "0101LB0ooobbbddd"},
   {arm7t_ldst_bh,             arm7t_jit_ldst_bh,             "SLDST[RD]",   "0101HS1ooobbbddd"},
   {arm9t_imm_off_ldst,        arm9t_jit_imm_off_ldst,        "LDST[IMM]",   "011BLOOOOObbbddd"},
   {arm7t_imm_off_ldst_bh,     arm7t_jit_imm_off_ldst_bh,     "SLDSTH[IMM]", "1000LOOOOObbbddd"},
   {arm9t_stack_off_ldst,      arm9t_jit_stack_off_ldst,      "LDST[SP]",    "1001LdddOOOOOOOO"},
   {arm7t_load_addr,           arm7t_jit_load_addr,           "LDADDR",      "1010SdddOOOOOOOO"},
   {arm7t_add_off_sp,          arm7t_jit_add_off_sp,          "SP+=OFF",     "10110000SOOOOOOO"},
   {arm9t_push_pop_reg,        arm9t_jit_push_pop_reg,        "PUSHPOPREG",  "1011L10Rllllllll"},
   {arm9t_mult_ldst,           arm9t_jit_mult_ldst,           "MLDST",       "1100Lbbbllllllll"},
   // Conditional branches cant branch on condition 1111
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "11010cccOOOOOOOO"},
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "110110ccOOOOOOOO"},
   {arm7t_cond_branch,         arm7t_jit_cond_branch,         "COND B",      "1101110cOOOOOOOO"},
